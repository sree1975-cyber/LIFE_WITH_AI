...
class DataLoader:
    ...
    def load_yfinance_data(self, symbol, period, start_date, end_date):
        ...
        for attempt in range(1, max_retries + 1):
            try:
                logger.info(
                    f"Attempt {attempt}: Downloading data for {symbol}")
                ...
                if data is None or data.empty:
                    logger.warning(
                        f"Attempt {attempt}: Empty data for {symbol}")
                ...
                logger.error(
                    f"Attempt {attempt} failed for {symbol}: {str(e)}")
                if attempt < max_retries:
                    sleep_time = backoff_factor**attempt
                    time.sleep(sleep_time)
                    continue
    ...
    def display_yfinance_interface():
        ...
        with col1:
            period_type = st.selectbox("Period Type",
                ...
                with st.spinner("Downloading data from YFinance..."):
                    try:
                        ...
                        if data is not None and not data.empty:
                            ...
                            # Process data
                            st.session_state.processed_data = data_processor.process_stock_data(
                                data)

                            st.success(
                                f"✅ Data downloaded successfully for {symbol}")
                        ...
                        logger.error(
                            f"Exception in yfinance download: {str(e)}")
...
# File Import UI
    ...
    if uploaded_file:
        ...
        st.markdown(
            "File must contain columns: Date (index), open, high, low, close, volume."
        )
    ...
    if st.button("📤 Process", key="process_file", type="primary"):
        try:
            with st.spinner("Processing uploaded file..."):
                st.session_state.data = data_loader.load_file_data(
                    uploaded_file)
                st.session_state.processed_data = process_stock_data(
    ...
    if data_source == "Yahoo Finance":
        try:
            ...
            if not hist_data.empty:
                st.info(
                    f"Data available from {hist_data.index[0].date()} to {hist_data.index[-1].date()}"
                )
            st.info(
                f"Period selected ranging from {st.session_state.data.index[0].date()} to {st.session_state.data.index[-1].date()}"
            )
        except:
            st.warning(
                "⚠️ Unable to fetch historical data range. Data may still be valid."
            )
...
if st.session_state.data is not None and not st.session_state.data.empty:
    ...
    export_format = st.selectbox("Export Format", ["CSV", "XLSX"],
                                 key="export_format")
